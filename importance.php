<!DOCTYPE html>
<html>
    
    <head>
        <meta charset = "utf-8" />
        <title>The Motive</title>
        <link href = "css/stylesheet.css" rel = "stylesheet" type = "text/css" />
    </head>
    
    <body style = "background-color: #4C636B; color: white;">
        <header>
            <h1><strong>The Motive Behind the Turing Machine</strong></h1>
             <nav>
                <a href = "person.php"><strong>The Mastermind</strong></a>
                <a href = "index.php" ><strong>Home</strong></a>
                <a href = "how.php"><strong>Components</strong></a>
            </nav>
            <hr>
        </header>
        <main>
            <section id = "container" style = "width: 49%" >
                <h3 class = "heading">Enigma Machine</h3>
                <div>
                    <ul>
                        <li class = "list"> During World War II the Germans had a great advantage over Britain which</li>
                        <li class = "list"> was the invention of the Enigma Machine. It was invented at the end of World War I by </li>
                        <li class = "list"> a German engineer, Arthur Scherbius. Many German Nazis used this as a weapon</li>
                        <li class = "list"> in order to encode messages that Britain couldn't brake. </li>
                        <li class = "list"> Which also introduced a new field of cryptology.</li>
                    </ul>
                </div>
                <img class = "machines" width = "300" height = "400" src = "img/Enigma_Machine2.jpeg" alt = "Enigma Machine" />
            </section>
            <section id = "container2" style = "width: 49%">
                <h3 class = "heading">Turing Machine(Bombe)</h3>
                <ul>
                    <li class = "list"> With Alan Turing's invention of the Turing Machine Britain was able to decode the Enigma messages.</li>
                    <li class = "list"> It was made for calculation purposes mainly and also to decode messages on paper tape.</li>
                    <li class = "list"> Alan had lost so much to get so far with this machine and he made it work.</li>
                    <li class = "list"> With his efforts we were able to create the computers we have today.</li>
                </ul>
                <img class = "machines" src = "img/Turing_Machine2.jpg" width = "500" height = "400" alt = "Turing Machine" />
            </section>
        </main>
    </body>
    
</html>